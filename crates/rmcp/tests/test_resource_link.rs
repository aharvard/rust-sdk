...
fn test_resource_link_in_tool_result() {
    ...
    assert!(json.contains("\"type\":\"resource_link\"") || json.contains("\"type\": \"resource_link\""));
    assert!(json.contains("\"uri\":\"file:///test/file.txt\"") || json.contains("\"uri\": \"file:///test/file.txt\""));
    ...
    let resource_link = deserialized.content[1].as_resource_link()
...
fn test_resource_link_with_full_metadata() {
    ...
    let resource_link = deserialized.content[0].as_resource_link()
    ...
    assert_eq!(resource_link.description, Some("JSON data from external API".to_string()));
    assert_eq!(resource_link.mime_type, Some("application/json".to_string()));
